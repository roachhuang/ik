...
class RobotArm2D:
    ...
    def __init__(self, **kwargs):
        ...
        self.joints = np.array([[self.xRoot, self.yRoot, 0, 1]],
                               dtype=np.float).T
        self.lengths = []
    ...
    def add_revolute_link(self, **kwargs):
        ...
        '''
        self.joints = np.append(self.joints,
                                np.array([[0, 0, 0, 1]]).T,
                                axis=1)
    ...
    def get_transformation_matrix(self, theta, x, y):
        ...
        transformationMatrix = np.array(
            [[math.cos(theta), -math.sin(theta), 0, x],
             [math.sin(theta), math.cos(theta), 0, y], [0, 0, 1, 0],
             [0, 0, 0, 1]])
    ...
    def update_joint_coords(self):
        ...
        T = self.get_transformation_matrix(self.thetas[0].item(), self.xRoot,
                                           self.yRoot)
    ...
    def get_jacobian(self):
        ...
        for i in range(len(self.joints[0, :]) - 1):
            ...
            jacobian[:, i] = np.cross(kUnitVec,
                                      (endEffectorCoords -
                                       currentJointCoords).reshape(3, ))
        return jacobian
